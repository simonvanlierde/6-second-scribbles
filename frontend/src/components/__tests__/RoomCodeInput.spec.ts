...
describe("RoomCodeInput", () => {
	...
	it("arrow keys do not throw", async () => {
		const wrapper = mountInput();
		await expect(
			getInput(wrapper, 1).trigger("keydown", { key: "ArrowLeft" }),
		).resolves.not.toThrow();
		await expect(
			getInput(wrapper, 0).trigger("keydown", { key: "ArrowRight" }),
		).resolves.not.toThrow();
	});
});
...
describe("HomeView", () => {
	...
	it("shows a toast when the room does not exist", async () => {
		...
		await wrapper.find(".btn.btn-join").trigger("click");
		await wrapper.vm.$nextTick();

		expect(wrapper.text()).toContain("That room doesn't exist");
