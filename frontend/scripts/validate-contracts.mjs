...
function validateEventSchemas(direction, schemaDirPath, examplesPath) {
  ...
  for (const [eventType, filename] of Object.entries(index)) {
    ...
    const matchingExamples = examples.filter((example) => example.payload?.type === eventType);
    ...
    for (const example of matchingExamples) {
      ...
      if (!result.success) {
        const message = result.error.issues.map((issue) => `${issue.path.join(".")}: ${issue.message}`).join("; ");
        throw new Error(`${direction} event schema ${eventType} rejected its example: ${message}`);
      }
    }
  }
}
...
function validateCatalogAgainstSchemas() {
  ...
  if (JSON.stringify(Object.keys(clientIndex).sort()) !== JSON.stringify(expectedClientTypes)) {
    throw new Error(`Client per-event schemas do not match contract catalog.`);
  }

  if (JSON.stringify(Object.keys(serverIndex).sort()) !== JSON.stringify(expectedServerTypes)) {
