...
export function useGameEngine() {
  ...
  function handleGuessSubmission(submission: GuessSubmission) {
    ...
    const allPlayersSubmitted = store.playersList.every((player) => guessSubmissions.has(player.id))
    ...
    for (const [playerId, submissions] of guessSubmissions.entries()) {
      for (const submission of submissions) {
        ...
        if (!targetPlayer || !targetPlayer.currentCard || !targetPlayer.currentCard.items) {
          console.warn(
            '[GameEngine] Skipping submission - no card or items for player:',
        ...
        newScores[submission.targetPlayerId] =
          (newScores[submission.targetPlayerId] || 0) + pointsEarned
